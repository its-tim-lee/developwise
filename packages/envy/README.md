...
That relationship is hard toㄑ keep in your head, so editing scripts and config becomes riskier than it should be. It also splits env access between `import.meta.env` in bundled runtime code and `process.env` in build/config code, which only creates another cognitive load.
